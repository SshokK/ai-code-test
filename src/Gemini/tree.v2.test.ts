...
describe('Gemini Tree V2', () => {
  ...
  describe('insert', () => {
    ...
    it('should return the new node even if the parent does not exist', () => {
      const newNode = tree.insert('B', 'NonExistentParent');
      expect(newNode).not.toBeNull(); // Correct assertion: It should return the new node.
      expect(newNode?.value).toBe('B'); // Check the value of the new node
      expect(newNode?.parent).toBeNull(); // Since parent doesn't exist, the parent of the node should be null
    });
